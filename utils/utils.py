...
def load_pretrained_ema(config, model, logger, model_ema=None):
    logger.info(f"=============> Loading weight {config.MODEL.PRETRAINED} for fine-tuning......")
    checkpoint = torch.load(config.MODEL.PRETRAINED, map_location='cpu')

    state_dict = checkpoint['model']
    # Xóa phần classifier cuối để tránh lỗi mismatch
    keys_to_remove = [k for k in state_dict.keys() if "head" in k or "classifier" in k]
    for k in keys_to_remove:
        logger.info(f"Removing key from pretrained weights: {k}")
        del state_dict[k]
    ...
    if model_ema is not None and 'model_ema' in checkpoint:
        logger.info("=> Loading EMA weights as well")
        try:
            model_ema.load_state_dict(checkpoint['model_ema'], strict=False)
        except:
            logger.warning("EMA weights loading failed")
