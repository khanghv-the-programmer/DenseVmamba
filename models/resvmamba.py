...
class VSSM(nn.Module):
    ...
    def forward(self, x: torch.Tensor):
        ...
        # print("X_patch_embed_dim: ", x.size()) #torch.Size([1, 56, 56, 96])
        channel_fuse_in  = [96, 192, 384, 768] #vssm_small
        channel_fuse_out = [192, 384, 768, 768] #vssm_small
        # print("self.layers: ", self.layers)
        for idx, layer in enumerate(self.layers) :
            # print("idx: ", idx)
            x_permuted = x.permute(0, 3, 1, 2) #torch.Size([1, 96, 56, 56])
            pool_x = nn.MaxPool2d(kernel_size=2, stride=2) 
            pool_x.to(device)
            cnt = 1
            if idx<3:
                while cnt <= (idx+1):
                    x_downsampled = pool_x(x_permuted) #[1, 96, 28, 28]
                    cnt += 1
                # print("x_downsampled: ", x_downsampled.size())
                conv1x1 = nn.Conv2d(in_channels=channel_fuse_in[idx], out_channels=channel_fuse_out[idx], kernel_size=1)
                conv1x1.to(device)
                x_resized = conv1x1(x_downsampled)
                # print("x_resized: ", x_resized.size())
                x_final = x_resized.permute(0, 2, 3, 1) 
                # print("x_final: ", x_final.size())
                y = layer(x) #torch.Size([1, 28, 28, 192]), torch.Size([1, 14, 14, 384]), torch.Size([1, 7, 7, 768]), torch.Size([1, 7, 7, 768])
                # print("Y_dim: ", y.size())
                ff = y + x_final
                x = ff
...
def check_vssm_equals_vmambadp():
    print("Run check_vssm_equals_vmambadp")
    try:
...
def check_vssm1_equals_vssm(ss2dfwd=SS2D.forward_corev0):
    print("Run check_vssm1_equals_vssm")
    try:
