...
def build_loader(config):
    ...
    # Nếu eval mode thì chỉ cần dataset val
    if config.EVAL_MODE:
        dataset_val, _ = build_dataset(is_train=False, config=config)
        config.freeze()
        print(f"[Eval mode] local rank {config.LOCAL_RANK} / global rank {dist.get_rank()} successfully built val dataset")
    ...
    # Nếu train mode thì load cả train + val
...
def build_dataset(is_train, config):
    ...
    if config.DATA.DATASET in ['imagenet', 'cnfood241']:
        ...
        # Tự lấy số class từ dataset nếu là ImageFolder
