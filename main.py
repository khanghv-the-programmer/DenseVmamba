...
def main(config):
    ...
    if config.EVAL_MODE:
        if config.MODEL.RESUME:
            load_checkpoint_ema(config, model_without_ddp, None, None, None, logger, model_ema)
        elif config.MODEL.PRETRAINED:
            load_pretrained_ema(config, model_without_ddp, logger, model_ema)

        acc1, acc5, loss = validate(config, data_loader_val, model)
        logger.info(f"[Eval] Accuracy on {len(dataset_val)} images: {acc1:.1f}%")
        if model_ema is not None:
            acc1_ema, acc5_ema, loss_ema = validate(config, data_loader_val, model_ema.ema)
            logger.info(f"[Eval] Accuracy EMA: {acc1_ema:.1f}%")
        return
    # ======================
    ...
    for epoch in range(config.TRAIN.START_EPOCH, config.TRAIN.EPOCHS):
        ...
        acc1, acc5, loss = validate(config, data_loader_val, model)
        logger.info(f"Accuracy@1: {acc1:.2f}% | Accuracy@5: {acc5:.2f}%")
        max_accuracy = max(max_accuracy, acc1)
        logger.info(f"Max Accuracy so far: {max_accuracy:.2f}%")

    if model_ema is not None:
        acc1_ema, acc5_ema, loss_ema = validate(config, data_loader_val, model_ema.ema)
        logger.info(f"[EMA] Accuracy@1: {acc1_ema:.2f}% | Accuracy@5: {acc5_ema:.2f}%")
        ...
        acc1, acc5, loss = validate(config, data_loader_val, model)
        logger.info(f"Accuracy: {acc1:.1f}%")
        max_accuracy = max(max_accuracy, acc1)
        logger.info(f"Max accuracy: {max_accuracy:.2f}%")

        if model_ema is not None:
            acc1_ema, acc5_ema, loss_ema = validate(config, data_loader_val, model_ema.ema)
            logger.info(f"EMA Accuracy: {acc1_ema:.1f}%")
            max_accuracy_ema = max(max_accuracy_ema, acc1_ema)
...
def train_one_epoch(config, model, criterion, data_loader, optimizer, epoch, mixup_fn, lr_scheduler, loss_scaler, model_ema=None):
    ...
    if mixup_fn is None:
        ...
        logger.info(f"[Train][Epoch {epoch}] Accuracy: {acc:.2f} | Recall: {recall:.2f} | F1: {f1:.2f}")
    else:
        logger.info(f"[Train][Epoch {epoch}] Skip metrics due to mixup.")
